...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  Object.keys(ctx.models).forEach(modelName => {
    //console.log(ctx.models[modelName].sharedClass.ctor.settings.sdk);
    if (ctx.models[modelName].sharedClass.ctor.settings.sdk &&
      ...
      */
      if (ctx.fireloopOnly === 'disabled' || (ctx.models[modelName].sharedClass.ctor.settings.base === 'User')) {
        console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', ctx.models[modelName].sharedClass.ctor.settings.base);
        schema.push({
   ...
   */
  function buildServiceImports(model, isIo, loadAccessToken) {
    ...
    if(modelName ==='Person'){
      imports.push({
          module: 'AccessToken', from: '../../models/BaseModels'
      });
    }

      if(isIo==='enabled'){
          imorts.push({ module: 'SocketConnections', from: '../../sockets/socket.connections'});
      }
  ...
  function buildModuleImports(models, isIo, driver) {
    ...
    if(isIo===true){
      imorts.push({ module: 'SocketDriver', from: './sockets/socket.driver'},{ module: 'SocketConnections', from: './sockets/socket.connections'});
    }
